...
class key_measurement_point_class(facility_component_class):
    ...
    def alarm_threshold2(self, facility):
        """
        Depending on KMP locations, expected alarm_thresholds are different.
        By using 0.05 % relative random and systematic errors for weight measurement, 
        the combined standard deviation can be calculated as below. 
        
        20 kg of a batch gets passed through vertex to vertex. 
        Therefore, 'amount' is an actual uncertainty amount of weight measurement. 
        
        While 2.4kg of SNM can be used as the expected standard error of MUF for proliferation resistance purpose,  
        other thresholds can also be tested.        
    
        """
        combined_sigma = math.sqrt(2*(19.75*0.05/100)**2) # 0.01396535892
        
        #combined_sigma_next = math.sqrt(2*(self.batch.weight*0.05/100)**2)
        # print combined_sigma # measurement errors; expected answer: 0.0141421356237
            
        # R - S without material losses
        sigma_no_loss = math.sqrt(combined_sigma**2 + combined_sigma**2)
        # print sigma_no_loss; expected answer: 0.02
        # print sigma_no_loss; 0.01975
        return 1.645*sigma_no_loss # 0.03248875
    ...
    def muf_check(self, facility): 
        ...
        if (self.identifier == 1):  
            ...
            if (facility.failure_check == True):
                ...
                if (self.muf > self.kmp_alarm_threshold1):
                    ...
                    self.muf_data_output1.write('%.4f\t%i\t%i\t%.4f\t%.4f\n'%(facility.operation_time, facility.total_campaign, facility.false_alarm_count_1, self.muf, self.kmp_alarm_threshold1))
        ...
        if (self.identifier == 2):
            ...
            self.muf = facility.kmp[self.identifier - 1].measured_weight - self.measured_weight  
            ...
            if (self.muf > self.kmp_alarm_threshold2):
                ...
                self.muf_data_output2.write('%.4f\t%i\t%i\t%.4f\t%.4f\n'%(facility.operation_time, facility.total_campaign, facility.false_alarm_count_2, self.muf, self.kmp_alarm_threshold2)) 
                #self.data_output.muf_output(facility, self)
        ...
        if (self.identifier == 3):
            ...
            if (self.muf > self.kmp_alarm_threshold_0_3):
                ...
                self.muf_data_output3.write('%.4f\t%i\t%i\t%.4f\t%.4f\n'%(facility.operation_time, facility.total_campaign, facility.false_alarm_count_3, self.muf, self.kmp_alarm_threshold_0_3)) 
                #self.data_output.muf_output(facility, self)
    ...
    def muf_check_2_melter(self, facility, previous_value): 
        ...
        if (self.muf > self.kmp_alarm_threshold_3_3):
           ...
           self.muf_data_output2.write('%.4f\t%i\t%i\t%.4f\t%.4f\n'%(facility.operation_time, facility.total_campaign, facility.false_alarm_count_3, self.muf, self.kmp_alarm_threshold_3_3)) 
                #self.data_output.muf_output(facility, self)
